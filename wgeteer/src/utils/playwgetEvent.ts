...
async function pptrEventSet(
  ...
  page.on('pageerror', (exception: any) => {
    // Safely access exception.location.url as it might be undefined in some cases (e.g., Firefox with certain internal errors)
    const errorLocation = exception.location
      ? ` at ${exception.location.url}`
      : '';
    logger.error(
      `[${pageId}] Uncaught exception: "${exception.message || exception}"${errorLocation}`,
    );
  });
  // download pdf
